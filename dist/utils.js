 ...
 * @returns
 */
export const initiateInstance = async (db, path) => {
    try {
        let uid;
        let offset;
 ...
 * @param uid
 */
export const deleteInstance = async (db, path, uid) => {
 ...
 */
export const killZombie = async (db, path, uid, peerUid) => {
    try {
        const ref = doc(db, `${path}/instances`, uid);
...
export const Uint8ArrayToBase64 = async (buffer) => {
...
export const generateKey = async (sender, receiver) => {
...
export const decryptData = async (message, key) => {
    try {
